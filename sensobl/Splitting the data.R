...
pathIlam <- "C:/Users/ebaninajar/Desktop/Sentinel2/Ilam2/Scenarios/Default/TxtInOut/"
q_obs_sarjoy <- read.csv("C:/Users/ebaninajar/Desktop/Sentinel2/Debi/Qsarjoy_final.csv", header = TRUE)
...
mat <-as.data.frame(sobol_matrices(N = n_samp, params = par_names)) %>% map2_dfc(., par_bound, ~ (.x * (.y[2] - .y[1]) + .y[1]))
 
set.seed(7)
for (i in 1:10){
picked <- sample(seq_len(nrow(mat)),size = 28000)
datasplit <- mat[picked,]
restofdata <- mat[-picked,]

filename =  sprintf(paste("d:\data_part",i,".csv", sep = ""))
write.csv(datasplit, file = filename)
mat <- restofdata
}